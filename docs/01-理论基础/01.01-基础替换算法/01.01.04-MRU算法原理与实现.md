...
1. **策略模式**：MRU算法作为缓存替换策略的一种实现
2. **队列模式**：使用队列数据结构实现MRU语义
...
// MRU策略实现
CacheReplacementStrategy mru_strategy = {
    .on_access = mru_on_access,
    .on_evict = mru_evict,
    .name = "MRU"
};
```
...
| 权衡维度 | 选择 | 原因 |
|---------|------|------|
| **性能 vs 准确性** | 简单MRU | 实现简单，性能高 |
| **简单 vs 复杂** | 队列实现 | 最简单的实现 |
| **通用 vs 专用** | 通用MRU实现 | 适用顺序扫描场景 |
