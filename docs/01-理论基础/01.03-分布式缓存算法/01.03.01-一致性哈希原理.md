...
Redis Cluster是一致性哈希在生产环境中的典型应用，使用16384个槽位（slot）来实现数据分片和负载均衡。
...
1. **槽位总数**：16384个槽位（相当于16384个虚拟节点）
2. **槽位分配**：每个节点负责一部分槽位，例如4个节点时，每个节点负责4096个槽位
3. **哈希函数**：使用CRC16算法计算key的哈希值，然后取模16384得到槽位号
4. **标签支持**：支持`{tag}`标签，相同标签的key会被分配到同一个槽位，便于批量操作

**槽位分配示例**：

```python
# Redis Cluster槽位分配示例
# 假设有4个节点：node1, node2, node3, node4

# 每个节点负责的槽位范围：
# node1: 0-4095 (4096个槽位)
# node2: 4096-8191 (4096个槽位)
# node3: 8192-12287 (4096个槽位)
# node4: 12288-16383 (4096个槽位)
...
1. **迁移触发**：节点添加、删除或重新平衡时触发槽位迁移
2. **迁移粒度**：以槽位为单位进行迁移，而不是单个key
3. **迁移过程**：
   - 标记槽位为迁移中状态
   - 逐个迁移槽位中的key
   - 更新槽位映射关系
...
# 迁移前：
# node1: 0-5460 (5461个槽位)
# node2: 5461-10922 (5462个槽位)
# node3: 10923-16383 (5461个槽位)
...
class ConsistentHash:
    ...
    def add_node(self, node):
        """添加节点到哈希环"""
        # 使用策略模式添加节点
        self.hash_strategy.add_node(node)
