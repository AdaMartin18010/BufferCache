...
Redis Cluster使用16384个槽位（slot）来实现虚拟节点优化，这是虚拟节点在实际生产环境中的典型应用。
...
```python
# 虚拟节点生成策略接口
class VirtualNodeStrategy:
    def generate(self, physical_node, count):
        raise NotImplementedError

class UniformStrategy(VirtualNodeStrategy):
    """均匀分布虚拟节点策略"""
    def generate(self, physical_node, count):
        """生成均匀分布的虚拟节点"""
        virtual_nodes = []
        for i in range(count):
            virtual_key = f"{physical_node}:vnode:{i}"
            virtual_nodes.append({
                'key': virtual_key,
                'hash': self._hash(virtual_key),
...
class WeightedStrategy(VirtualNodeStrategy):
    """加权分布虚拟节点策略"""
    def generate(self, physical_node, count, weight):
        """根据权重生成虚拟节点"""
        virtual_nodes = []
        # 根据权重调整虚拟节点数
        actual_count = int(count * weight)
...
class VirtualNodeManager:
    def __init__(self, strategy: VirtualNodeStrategy):
        self.strategy = strategy
        self.virtual_nodes = {}

    def add_physical_node(self, node, count):
        virtual_nodes = self.strategy.generate(node, count)
        for vnode in virtual_nodes:
            self.virtual_nodes[vnode] = node
```
