...
Redis使用引用计数（Reference Counting）作为内存管理机制，通过计数对象的引用次数来决定何时释放内存。
理解引用计数GC对于优化Redis内存使用和性能至关重要。
...
| 权衡维度 | 选择 | 原因 |
|---------|------|------|
| **性能 vs 内存** | 性能优先 | 引用计数GC性能高 |
| **简单 vs 复杂** | 引用计数 | 实现相对简单 |
| **通用 vs 专用** | 通用引用计数GC实现 | 适用多种场景 |
...
- $C_{performance}$：性能成本（引用计数操作，性能高）
- $C_{memory}$：内存成本（引用计数字段，内存开销）
- $C_{complexity}$：复杂度成本（引用计数管理，复杂度中等）
...
typedef struct gc_manager {
    void (*incr_ref)(void *obj);
    void (*decr_ref)(void *obj);
...
- **代码清晰**：引用计数GC逻辑清晰，易于理解
- **易于调试**：引用计数状态易于监控和调试
- **测试友好**：引用计数GC行为易于测试和验证
