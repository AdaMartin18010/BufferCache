...
- [05.04.01 请求-响应数据流](#050401-请求-响应数据流)
  - [目录](#目录)
  - [1. 概述](#1-概述)
  ...
  - [6. 响应返回阶段](#6-响应返回阶段)
    ...
    - [6.2 数据发送](#62-数据发送)
  - [7. 性能优化](#7-性能优化)
    - [7.1 Pipeline批处理](#71-pipeline批处理)
    - [7.2 响应缓冲](#72-响应缓冲)
    - [7.3 写事件延迟注册](#73-写事件延迟注册)
  - [8. 延迟分解](#8-延迟分解)
    - [8.1 各阶段延迟](#81-各阶段延迟)
  - [9. 程序设计分析](#9-程序设计分析)
...
typedef struct data_pipeline {
    ...
    int stage_count;
    void (*process)(struct data_pipeline *pipeline, request_t *req);
} data_pipeline_t;

// 管道阶段
typedef struct pipeline_stage {
    void (*process)(request_t *req, response_t *resp);
...
| 权衡维度 | 选择 | 原因 |
|---------|------|------|
| **性能 vs 可维护性** | 平衡 | 平衡性能和可维护性 |
| **简单 vs 复杂** | 管道处理 | 需要理解数据流各阶段 |
| **通用 vs 专用** | 通用数据流实现 | 适用多种场景 |

**权衡公式**：

$$C_{total} = C_{performance} + C_{maintainability} + C_{complexity}$$

其中：

- $C_{performance}$：性能成本（数据流处理开销）
- $C_{maintainability}$：可维护性成本（管道设计，可维护性好）
- $C_{complexity}$：复杂度成本（数据流，复杂度较高）
...
typedef struct data_flow {
    data_pipeline_t *pipeline;
    void (*add_stage)(struct data_flow *flow, pipeline_stage_t *stage);
    void (*process)(struct data_flow *flow, request_t *req);
} data_flow_t;
```
