...
class ClusterFailureDetector:
    ...
    def send_alert(self, failure):
        ...
        try:
            ...
            # 发送到告警系统（可以通过多种渠道）
            # 1. 记录到Redis供监控系统抓取
            alert_key = f"cluster:alert:{failure.get('id')}"
            self.redis.setex(
                alert_key,
                3600,  # 1小时过期
            ...
            # 3. 发送到监控系统（Prometheus、Grafana等）
            self._send_to_monitoring(alert_message)
    ...
    def trigger_failover(self, node_id):
        ...
        try:
            ...
            if node_info.get('role') != 'master':
                print(f"Node {node_id} is not a master node, skipping failover")
                return False

            # 获取从节点列表
            replica_nodes = self.get_replica_nodes(node_id)
            if not replica_nodes:
                raise ValueError(f"No replica nodes found for master {node_id}")

            # 选择最佳从节点进行提升
            best_replica = self._select_best_replica(replica_nodes)
            ...
            success = self._execute_failover(node_id, best_replica)

            if success:
                # 发送故障转移通知
                self.send_alert({
                    'id': f"failover-{node_id}",
                    'node_id': node_id,
                    'type': 'failover',
                    'severity': 'critical',
                    'message': f"Failover triggered: {node_id} -> {best_replica}"
                })
    ...
    def _select_best_replica(self, replica_nodes):
        """选择最佳从节点"""
        # 选择延迟最低、数据最新的从节点
        best_replica = None
        min_lag = float('inf')
        ...
        for replica in replica_nodes:
            lag = replica.get('replication_lag', float('inf'))
            if lag < min_lag:
                min_lag = lag
                best_replica = replica['node_id']

        return best_replica or replica_nodes[0]['node_id']

    def _execute_failover(self, master_id, replica_id):
        """执行故障转移"""
        try:
            # 使用Redis Sentinel或Cluster的故障转移命令
            # 这里简化处理，实际应该调用Redis的故障转移API

            # 1. 标记主节点为故障
            self.mark_node_failed(master_id)

            # 2. 提升从节点为主节点
            self.promote_replica_to_master(replica_id)

            # 3. 更新集群配置
            self.update_cluster_config(master_id, replica_id)

            return True
        except Exception as e:
            print(f"Failed to execute failover: {e}")
            return False
```
