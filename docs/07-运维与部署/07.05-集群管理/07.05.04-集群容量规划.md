    ...
    def get_data_size(self):
        ...
        try:
            ...
            for node in self.cluster.nodes.values():
                try:
                    # 获取节点的数据库信息
                    info = node.execute_command('INFO', 'keyspace')

                    # 解析keyspace信息
                    # 格式：db0:keys=1000,expires=100,avg_ttl=3600
                        ...
                        if line.startswith('db'):
                            ...
                            for part in parts:
                                if part.startswith('keys='):
                                    ...
                                    # 估算每个键的平均大小（字节）
                                    avg_key_size = 1024  # 假设平均1KB
                                    total_size += keys * avg_key_size

                    # 或者使用DBSIZE命令获取键数量
                    # db_size = node.dbsize()
                    # total_size += db_size * avg_key_size

                except Exception as e:
                    print(f"Error getting data size from node {node}: {e}")
                    continue
    ...
    def get_qps(self):
        ...
        try:
            ...
            for node in self.cluster.nodes.values():
                try:
                    # 获取节点的统计信息
                    stats = node.execute_command('INFO', 'stats')

                    # 解析统计信息
                    # 格式：total_commands_processed:123456
                    for line in stats.split('\n'):
                        if line.startswith('total_commands_processed:'):
                            commands = int(line.split(':')[1])

                            # 获取上次统计值
                            last_key = f"node:qps:last:{node.name}"
                            last_time_key = f"node:qps:time:{node.name}"

                            last_commands = self.cluster.get(last_key)
                            ...
                            if last_commands and last_time_str:
                                ...
                                if time_diff > 0:
                                    node_qps = (commands - last_commands) / time_diff
                                    total_qps += node_qps

                            # 更新统计值
                            self.cluster.set(last_key, commands)
                            self.cluster.set(last_time_key, datetime.utcnow().isoformat())

                except Exception as e:
                    print(f"Error getting QPS from node {node}: {e}")
                    continue
    ...
    def get_memory_usage(self):
        ...
        try:
            ...
            for node in self.cluster.nodes.values():
                try:
                    # 获取节点的内存信息
                    info = node.execute_command('INFO', 'memory')

                    # 解析内存信息
                    # 格式：used_memory:1234567890
                    #      used_memory_human:1.15G
                    node_total_memory = None
                    node_used_memory = None

                    for line in info.split('\n'):
                        if line.startswith('used_memory:'):
                            node_used_memory = int(line.split(':')[1])
                        elif line.startswith('maxmemory:'):
                            max_mem = line.split(':')[1]
                            if max_mem and max_mem != '0':
                                node_total_memory = int(max_mem)

                    # 如果没有设置maxmemory，使用系统内存
                    if node_total_memory is None:
                        # 尝试从系统获取
                        import psutil
                        node_total_memory = psutil.virtual_memory().total

                    if node_used_memory and node_total_memory:
                        total_memory += node_total_memory
                        used_memory += node_used_memory

                except Exception as e:
                    print(f"Error getting memory usage from node {node}: {e}")
                    continue
...
def plan_cluster_capacity(data_size, qps):
    ...
    # 容量规划参数
    # 每个节点建议的数据量（GB）
    MAX_DATA_PER_NODE_GB = 100
    # 每个节点建议的QPS
    MAX_QPS_PER_NODE = 50000
    # 冗余系数（考虑主从复制）
    REDUNDANCY_FACTOR = 2

    # 计算所需节点数（数据量）
    data_size_gb = data_size / (1024 ** 3)
    nodes_by_data = int((data_size_gb / MAX_DATA_PER_NODE_GB) * REDUNDANCY_FACTOR)
    nodes_by_data = max(1, nodes_by_data)  # 至少1个节点

    # 计算所需节点数（QPS）
    nodes_by_qps = int((qps / MAX_QPS_PER_NODE) * REDUNDANCY_FACTOR)
    nodes_by_qps = max(1, nodes_by_qps)  # 至少1个节点

    # 取较大值
    required_nodes = max(nodes_by_data, nodes_by_qps)

    # 计算槽位分配
    slots_per_node = 16384 // required_nodes
    remaining_slots = 16384 % required_nodes

    # 分配槽位
    slot_allocation = []
    slot_start = 0
    for i in range(required_nodes):
        node_slots = slots_per_node + (1 if i < remaining_slots else 0)
        slot_allocation.append({
            'node_id': i + 1,
            'slot_range': (slot_start, slot_start + node_slots - 1),
            'slots': node_slots
        })
        slot_start += node_slots

    return {
        'nodes': required_nodes,
        'data_size': data_size,
        'qps': qps,
        'nodes_by_data': nodes_by_data,
        'nodes_by_qps': nodes_by_qps,
        'slot_allocation': slot_allocation,
        'recommendations': {
            'min_nodes': required_nodes,
            'recommended_nodes': required_nodes + 2,  # 建议多2个节点作为缓冲
            'max_data_per_node_gb': MAX_DATA_PER_NODE_GB,
            'max_qps_per_node': MAX_QPS_PER_NODE
        }
    }
```
