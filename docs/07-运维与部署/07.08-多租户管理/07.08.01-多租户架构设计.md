...
- [07.08.01 多租户架构设计](#070801-多租户架构设计)
  ...
  - [11. 租户管理API](#11-租户管理api)
    - [11.1 租户创建](#111-租户创建)
    - [11.2 租户配置管理工具](#112-租户配置管理工具)
    - [11.3 租户删除工具](#113-租户删除工具)
  - [12. 大规模多租户实践](#12-大规模多租户实践)
    - [12.1 1000+租户管理](#121-1000租户管理)
      - [12.1.1 租户批量管理工具](#1211-租户批量管理工具)
    - [12.2 租户监控工具](#122-租户监控工具)
      - [12.2.1 租户资源使用监控](#1221-租户资源使用监控)
  - [13. 最佳实践](#13-最佳实践)
...
class TenantIdentifier:
    ...
    def parse_token(token):
        ...
        try:
            ...
            # 从Token中提取租户ID
            tenant_id = decoded_token.get('tenant_id') or decoded_token.get('sub')
...
def get_redis_client(tenant_id):
    ...
    # 根据隔离策略返回不同的客户端
    isolation_type = tenant.get('isolation_type', 'key_prefix')

    if isolation_type == 'key_prefix':
        # 键前缀隔离：使用共享Redis实例
        return redis.Redis(
            host='shared-redis',
            port=6379,
        ...
        # 数据库隔离：使用不同的数据库编号
        db_number = tenant.get('db_number', 0)
        return redis.Redis(
            host='shared-redis',
            port=6379,
            db=db_number,
    ...
    elif isolation_type == 'instance':
        # 实例隔离：使用独立的Redis实例
        instance_config = tenant.get('instance_config', {})
        return redis.Redis(
            host=instance_config.get('host', 'localhost'),
            port=instance_config.get('port', 6379),
...
class ResourceMonitor:
    ...
    def get_tenant_memory_usage(self, tenant_id):
        ...
        try:
            ...
            # 获取内存使用量
            info = tenant_client.info('memory')
            memory_usage = info.get('used_memory', 0)

            # 如果是键前缀隔离，需要计算租户键的内存使用
            tenant_config = self.tenant_configs.get(tenant_id, {})
            if tenant_config.get('isolation_type') == 'key_prefix':
                # 扫描租户的所有键，计算总内存
                ...
                while True:
                    cursor, keys = tenant_client.scan(
                        cursor=cursor,
                        match=f"{tenant_prefix}*",
                        count=100
                    )

                    for key in keys:
                        try:
                            key_memory = tenant_client.memory_usage(key)
            ...
            print(f"Error getting tenant memory usage: {e}")
            return 0
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                if 'redis' in proc.info['name'].lower():
                    redis_process = psutil.Process(proc.info['pid'])
                    break

            if redis_process:
                # 获取CPU使用率（百分比）
                cpu_percent = redis_process.cpu_percent(interval=1)

                # 如果是实例隔离，直接返回
                tenant_config = self.tenant_configs.get(tenant_id, {})
                if tenant_config.get('isolation_type') == 'instance':
                    return cpu_percent

                # 如果是键前缀或数据库隔离，需要按比例分配
                # 这里简化处理，返回总CPU使用率
                return cpu_percent

            return 0.0
        except Exception as e:
            print(f"Error getting tenant CPU usage: {e}")
            return 0.0
    ...
    def get_tenant_connections(self, tenant_id):
        ...
        try:
            ...
            # 获取连接信息
            info = tenant_client.info('clients')
            ...
            # 如果是键前缀或数据库隔离，需要统计租户的连接数
            # 这里简化处理，使用CLIENT LIST命令统计
            client_list = tenant_client.client_list()
            ...
            for client in client_list:
                ...
                client_db = client.get('db', '0')

                if tenant_prefix in client_name or \
                   (tenant_config.get('isolation_type') == 'database' and
                    client_db == str(tenant_config.get('db_number', 0))):
                    tenant_connections += 1

            return tenant_connections
        except Exception as e:
            print(f"Error getting tenant connections: {e}")
            return 0

    def get_tenant_qps(self, tenant_id):
        """获取租户QPS"""
        try:
            # 使用Redis的INFO stats命令获取命令统计
            tenant_client = self.get_tenant_client(tenant_id)

            # 获取统计信息
            stats = tenant_client.info('stats')
            total_commands = stats.get('total_commands_processed', 0)

            # 计算QPS（需要记录上次的统计值和时间）
            current_time = time.time()
            last_key = f"tenant:qps:last:{tenant_id}"
            last_time_key = f"tenant:qps:time:{tenant_id}"

            last_commands = tenant_client.get(last_key)
            last_time = tenant_client.get(last_time_key)

            if last_commands and last_time:
                time_diff = current_time - float(last_time)
                commands_diff = total_commands - int(last_commands)

                if time_diff > 0:
                    qps = commands_diff / time_diff
                else:
                    qps = 0
            else:
                qps = 0
            ...
            # 更新记录
            tenant_client.set(last_key, total_commands)
            tenant_client.set(last_time_key, current_time)

            return qps
        except Exception as e:
            print(f"Error getting tenant QPS: {e}")
            return 0.0
```
    ...
    def _set_memory_quota(self, tenant_id, memory_mb):
        ...
        if tenant_config.get('isolation_type') == 'instance':
            try:
                ...
                print(f"Failed to set memory quota: {e}")

        # 在Redis中存储配额配置
        quota_key = f"tenant:quota:memory:{tenant_id}"
        self.cluster.set(quota_key, memory_bytes)

    def _set_qps_limit(self, tenant_id, qps_limit):
        """设置QPS限制"""
        # 在Redis中存储QPS限制配置
        qps_key = f"tenant:quota:qps:{tenant_id}"
        self.cluster.set(qps_key, qps_limit)

        # 如果使用限流器，更新限流器配置
        if hasattr(self, 'rate_limiter'):
            self.rate_limiter.set_limit(tenant_id, qps_limit)
...
class TenantDeletionTool:
    ...
    def _get_tenant_database(self, tenant_id):
        ...
        if tenant_config.get('isolation_type') == 'database':
            return tenant_config.get('db_number', 0)
    ...
    def _get_tenant_instance(self, tenant_id):
        ...
        if tenant_config.get('isolation_type') == 'instance':
            return tenant_config.get('instance_config', {})

        # 如果使用键前缀或数据库隔离，返回None
        return None
    ...
    def _delete_instance(self, instance_info):
        ...
        try:
            ...
            if not host or not port:
                raise ValueError("Instance info must contain host and port")

            # 连接到实例
            instance_client = redis.Redis(
                host=host,
                port=port,
            ...
            # 清空所有数据（可选）
            if instance_info.get('clear_data', False):
                instance_client.flushall()
            ...
            # container_name = f"redis-{instance_info.get('instance_id')}"
...
class TenantResourceMonitor:
    ...
    def _get_tenant_qps(self, tenant_id):
        ...
        try:
            ...
            # 获取上次统计值
            last_stats_key = f"tenant:qps:last:{tenant_id}"
            last_time_key = f"tenant:qps:time:{tenant_id}"

            last_commands = self.cluster.get(last_stats_key)
            last_time_str = self.cluster.get(last_time_key)

            if last_commands and last_time_str:
                last_commands = int(last_commands)
                ...
                if time_diff > 0:
                    # 计算QPS
                    commands_diff = total_commands - last_commands
                    qps = commands_diff / time_diff
                else:
                    qps = 0
            else:
            ...
            # 更新统计值
            self.cluster.set(last_stats_key, total_commands)
            self.cluster.set(last_time_key, current_time.isoformat())

            # 记录QPS历史（用于图表展示）
            qps_history_key = f"tenant:qps:history:{tenant_id}"
            qps_data = {
            ...
            # 使用列表存储历史数据（保留最近100条）
            self.cluster.lpush(qps_history_key, json.dumps(qps_data))
            self.cluster.ltrim(qps_history_key, 0, 99)
            self.cluster.expire(qps_history_key, 3600)  # 1小时过期

            return qps

        except Exception as e:
            print(f"Error calculating QPS: {e}")
            return 0.0
        return 0
    ...
    def _get_tenant_instance(self, tenant_id):
        ...
        try:
            # 从租户配置中获取实例信息
            tenant_config_key = f"tenant:config:{tenant_id}"
            config_json = self.cluster.get(tenant_config_key)

            if config_json:
                config = json.loads(config_json)
                if config.get('isolation_type') == 'instance':
                    return config.get('instance_config', {})

            return None
        except Exception as e:
            print(f"Error getting tenant instance: {e}")
            return None
    ...
    def _get_instance_stats(self, instance_info):
        ...
        try:
            ...
            # 连接到实例
            instance_client = redis.Redis(
                host=instance_info.get('host', 'localhost'),
                port=instance_info.get('port', 6379),
                password=instance_info.get('password'),
                decode_responses=True
            )

            # 获取统计信息
            memory_info = instance_client.info('memory')
            stats_info = instance_client.info('stats')
            keyspace_info = instance_client.info('keyspace')
            ...
            stats = {
                ...
                'memory_ratio': memory_info.get('mem_fragmentation_ratio', 0),
                'total_keys': total_keys,
                'total_commands': stats_info.get('total_commands_processed', 0),
                'instantaneous_ops': stats_info.get('instantaneous_ops_per_sec', 0),
