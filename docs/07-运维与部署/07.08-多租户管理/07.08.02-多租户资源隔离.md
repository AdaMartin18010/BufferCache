...
class MemoryQuotaManager:
    ...
    def get_tenant_client(self, tenant_id):
        ...
        # 根据隔离策略返回不同的客户端
        isolation_type = tenant_config.get('isolation_type', 'key_prefix')

        if isolation_type == 'key_prefix':
            # 键前缀隔离：使用同一个Redis实例，通过键前缀区分
            return self.redis_client
        elif isolation_type == 'database':
            # 数据库隔离：使用不同的数据库编号
            db_number = tenant_config.get('db_number', 0)
            return redis.Redis(
            ...
            # 实例隔离：使用独立的Redis实例
            instance_config = tenant_config.get('instance_config', {})
            return redis.Redis(
...
class MemoryOverlimitHandler:
    ...
    def cleanup_lru_keys(self, tenant_id):
        """清理LRU键"""
        # 获取租户内存使用量
        tenant_memory = self.quota_manager.get_tenant_memory_usage(tenant_id)
        tenant_limit = self.quota_manager.get_tenant_memory_limit(tenant_id)

        if tenant_memory <= tenant_limit:
            return 0

        # 需要清理的内存量
        excess_memory = tenant_memory - tenant_limit
        ...
        while True:
            ...
            for key in keys:
                ...
                memory = self.redis.memory_usage(key)
                keys_with_idle.append((key, idle_time, memory))
        ...
        # 删除最久未使用的键，直到内存使用量降到限制以下
        deleted_count = 0
        freed_memory = 0

        for key, idle_time, memory in keys_with_idle:
    ...
    def send_alert(self, tenant_id, info):
        ...
        alert_message = {
            'tenant_id': tenant_id,
            'alert_type': 'memory_overlimit',
            'timestamp': time.time(),
            'memory_usage': info.get('memory_usage', 0),
            'memory_limit': info.get('memory_limit', 0),
            'excess_memory': info.get('excess_memory', 0)
        }

        # 发送告警（可以通过邮件、短信、Webhook等方式）
        # 这里使用日志记录作为示例
        import logging
        logger = logging.getLogger(__name__)
        logger.warning(f"Memory quota exceeded for tenant {tenant_id}: {alert_message}")

        # 可以集成告警系统，如：
        # - 发送邮件告警
        # - 调用Webhook
        # - 发送到监控系统（Prometheus、Grafana等）
        # - 发送到消息队列（Kafka、RabbitMQ等）
...
if qps_limiter.check_qps('tenant1'):
    # 允许请求，执行业务逻辑
    result = process_request('tenant1', request_data)
    return result
else:
    # 拒绝请求，返回错误
    raise QPSLimitExceededError("QPS limit exceeded for tenant1")
```
...
class UnifiedResourceMonitor:
    ...
    def get_tenant_usage(self, tenant_id) -> Dict:
        ...
        if isolation_type == 'key_prefix':
            ...
            db_info = tenant_client.info('keyspace')

            usage = {
                'memory_usage': info.get('used_memory', 0),
                'key_count': db_info.get(f'db{db_number}', {}).get('keys', 0),
                'isolation_type': isolation_type
            }

        elif isolation_type == 'instance':
            # 实例隔离：使用INFO命令获取实例使用量
            instance_config = tenant_config.get('instance_config', {})
            tenant_client = redis.Redis(
                host=instance_config.get('host', 'localhost'),
                port=instance_config.get('port', 6379),
...
class ResourceAlertSystem:
    ...
    def _send_critical_alert(self, alert):
        """发送严重告警"""
        print(f"[CRITICAL] Tenant: {alert['tenant_id']}, {alert['message']}")
    ...
    def _send_warning_alert(self, alert):
        """发送警告告警"""
        print(f"[WARNING] Tenant: {alert['tenant_id']}, {alert['message']}")
    ...
    def _send_email(self, alert):
        ...
        try:
            ...
            receiver_email = f"admin-{alert['tenant_id']}@example.com"

            # 构建邮件内容
            message = MIMEText(
                f"租户资源告警\n\n"
                f"租户ID: {alert['tenant_id']}\n"
                f"告警类型: {alert['type']}\n"
                f"严重程度: {alert['severity']}\n"
                f"使用率: {alert['usage_ratio']*100:.2f}%\n"
                f"消息: {alert['message']}\n"
                f"时间: {alert.get('timestamp', datetime.utcnow().isoformat())}",
                'plain',
                'utf-8'
            )
            message['From'] = sender_email
            message['To'] = receiver_email
            message['Subject'] = f"[{alert['severity'].upper()}] 租户资源告警 - {alert['tenant_id']}"
    ...
    def _send_to_monitoring(self, alert):
        """发送到监控系统"""
        try:
            import json
            # 记录到Redis，供Prometheus等监控系统抓取
            alert_key = f"tenant:alert:{alert['tenant_id']}:{alert['type']}"
            self.cluster.setex(
                alert_key,
                3600,  # 1小时过期
                json.dumps({
            ...
            print(f"Failed to send to monitoring: {e}")
    ...
    def _get_tenant_usage(self, tenant_id):
        ...
        try:
            ...
            usage_data = self.cluster.get(usage_key)
    ...
    def _get_tenant_quota(self, tenant_id):
        ...
        try:
            import json
            # 从Redis获取租户配额
            quota_key = f"tenant:quota:{tenant_id}"
            quota_data = self.cluster.get(quota_key)
            ...
            print(f"Failed to get tenant quota: {e}")
            return {}
    ...
    def _list_all_tenants(self):
        ...
        try:
            ...
            while True:
                ...
                for key in keys:
                    # 提取租户ID：tenant:quota:tenant_id -> tenant_id
